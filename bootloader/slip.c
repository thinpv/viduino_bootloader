...
{
  printf("direction: 0x%02X, command: 0x%02X, size: %d, checksum: 0x%08lX", 
  ...
  if(slip_message_ptr->data && slip_message_ptr->size > 0) {
    if(slip_message_ptr->size <= 16) {
      ...
      for(int i=0; i<slip_message_ptr->size; i++) {
        if(i%4 == 0)
          printf(" ");
        printf("%02X", slip_message_ptr->data[i]);
      }
      printf("\r\n");
    } else if(slip_message_ptr->size < 2560) {
      printf("\r\n");
