...
int main(int argc, char * argv[])
{
	...
	len = bllen;//LZ4_compressBound(bllen);
	zbllen = rsize + sizeof(struct zdesc_t) + len;
	zblbuf = malloc(zbllen);
	memset(zblbuf, 0, zbllen);
	memcpy(&zblbuf[0], &blbuf[0], rsize);
	memcpy(&zblbuf[rsize + sizeof(struct zdesc_t)], &blbuf[0], len);
	clen = len;//LZ4_compress_HC(&blbuf[0], &zblbuf[rsize + sizeof(struct zdesc_t)], bllen, len, 12);
