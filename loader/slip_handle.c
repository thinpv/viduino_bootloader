...
static uint32_t slip_handle_read_reg(uint32_t addr)
{
	switch (addr)
	{
	case CHIP_DETECT_MAGIC_REG_ADDR: // 00100040
...
uint32_t slip_handle_flash_begin_message(slip_message_st *slip_message_src, slip_message_st *slip_message_des)
{
	...
	SLIP_LOG("erase_size: %ld, number_of_packet: %ld, package_size: %ld, offset: 0x%08lX\r\n",
	...
	sys_spi_flash_erase_4k(slip_flash_begin_message.offset - SIZE_4K);
	SLIP_LOG("write 0x%08lX to 0x%08X\r\n", slip_flash_begin_message.erase_size, slip_flash_begin_message.offset - SIZE_4K);
	sys_spi_flash_write(slip_flash_begin_message.offset - SIZE_4K, (uint8_t *)&slip_flash_begin_message.erase_size, 4);
...
uint32_t slip_handle_flash_data_message(slip_message_st *slip_message_src, slip_message_st *slip_message_des)
{
	...
	SLIP_LOG("data_size: %ld, sequence_number: %ld, crc_cal: 0x%08lX\r\n",
	...
	*((uint32_t *)slip_message_des->data) = 0;
	...
	SLIP_LOG("Received message is invalid ==============================================>\r\n");
	slip_message_des->data[0] = 1;
